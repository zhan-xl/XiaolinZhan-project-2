...
import {text} from '../resources/commonSevenWords.json';
import Hint from "../components/hint";

export default function GameDifficult(prop) {
  let [answer, setAnswer] = useState('');
  let [words, setWords] = useState([]);
  const [attempt, setAttempt] = useState(6);
  const [guess, setGuess] = useState('');
  const [message, setMessage] = useState('Welcome, this is difficult level.');
  ...
  function handleCheck() {
    if (!isNumberOfLetter()) {
      setMessage('Please enter a word with 7 letter.');
      return null;
    }
    if (!isValidWord()) {
